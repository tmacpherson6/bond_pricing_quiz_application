    ...
    for _, row in df.iterrows():
        ...
        if random_column == "pv_with_coupon":
            question = (
                ...
                f"The discount rate"
                f" is estimated by the function:  0.15 + "
                f"0.2 * (1 - np.exp(-0.2 * time)). "
            ...
            question = (
                ...
                f"interest rate of {row['coupon_rate']:.2%}. The discount rate"
                f" is estimated by the function:  0.15 + 0.2 * "
                f"(1 - np.exp(-0.2 * time)). What is the "
            ...
            question = (
                ...
                f"It matures in {row['years_to_maturity']} years and has an "
                f"annual interest rate of {row['coupon_rate']:.2%}. "
                f"The discount rate"
                f" is estimated by the function:  0.15 + "
                f"0.2 * (1 - np.exp(-0.2 * time)) .What is the "
                f"estimated 'face value' of this bond?"
            )
        elif random_column == "coupon_rate":
            question = (
                f"\n{row['question_id']} - A bond is worth "
                f"${row['pv_with_coupon']:.2f} today and has face value of "
                f"${row['face_value']}. It matures in "
                f"{row['years_to_maturity']} "
                f"years and pays {row['coupon_payment_schedule']} coupons per "
                f"year. The discount rate"
                f" is estimated by the function:  "
                f"0.15 + 0.2 * (1 - np.exp(-0.2 * time)) . What "
                f"is the bond's 'annual yield'?"
            )
